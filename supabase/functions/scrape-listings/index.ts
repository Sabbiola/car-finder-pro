...
function detectFuel(text: string): string | null {
  ...
  // Engine codes → Diesel
  if (/\b(tdi|cdi|crdi|hdi|dci|jtd|jtdm|mjet|cdti|bluehdi|bluedci|d4d|d-4d|dcat)\b/i.test(text))
    return "Diesel";
  // Engine codes → Benzina
  if (/\b(tfsi|tsi|gti|vti|sce|tce|puretech|t-?jet|gdi|mair|t-gdi|mpi)\b/i.test(text))
    return "Benzina";
  // Named EV models → Elettrica
  if (
    /\be-tron\b|\bid\.\d|\b(ioniq|niro)\s*ev\b|\bzoe\b|\bleaf\b|\be-208\b|\be-2008\b|\beariya\b/i.test(
      text,
    )
  )
    return "Elettrica";
...
function detectBodyType(text: string): string | null {
  ...
  if (t.includes("gran coupé") || t.includes("gran coupe") || t.includes("gran turismo"))
    return "Gran Coupé";
  if (t.includes("coupé") || t.includes("coupe")) return "Coupé";
  if (
    t.includes("cabrio") ||
    t.includes("convertible") ||
    t.includes("cabriolet") ||
    t.includes("spider") ||
    t.includes("roadster")
  )
    return "Cabrio";
  ...
  if (
    ...
    t.includes("fuoristrada") ||
    t.includes("4x4")
  )
    return "SUV";
  if (t.includes("monovolume") || t.includes("minivan") || /\b(van|mpv)\b/.test(t))
    return "Monovolume";
  return null;
}
...
function extractTrimFromTitle(title: string, brand: string, model: string): string | null {
  ...
  const trimKeywords =
    /\b(Sport\s*Line|M\s*Sport|xDrive|S\s*Line|AMG|Avantgarde|Elegance|Executive|Urban|Night|Plus|Premium|Business|Style|Active|Competition|GT|GTS|Luxury|Pack|Edition|Sport\s*Plus|e-Power)\b/i;
...
function parseSubitoListings(
  ...
  for (const block of blocks) {
    ...
    const urlMatch =
      ...
      block.match(
        /(https:\/\/www\.subito\.it\/(?:auto|auto-usate)\/[^\s)>\]"]+\.htm\b[^\s)>\]"]*)/,
      ) ||
      block.match(/(https:\/\/www\.subito\.it\/(?:auto|auto-usate)\/[^\s)>\]"]+)/);
...
// ==========================================
function parseAutoScoutListings(
  markdown: string,
  brand: string,
  model: string,
  trim: string | null,
  ...
  for (const block of parts) {
    const imgMatch = block.match(
      /!\[[^\]]*\]\((https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/([a-f0-9\-]{36})[^\s)]*)\)/,
    );
    ...
    const linkTitleMatch = block.match(
      /\[([^\]]{5,100}?)\]\(https:\/\/www\.autoscout24\.it\/annunci\//,
    );
    ...
    if (!title) {
      for (const line of block.split("\n")) {
        const clean = line
          ...
          .trim();
        if (
          (!brand || clean.toLowerCase().includes(brandPrefix)) &&
          clean.length >= 5 &&
          clean.length <= 120
        ) {
          title = clean;
    ...
    if (dateMatch) year = parseInt(dateMatch[2]);
    if (!year) {
      const yf = block.match(/\b(20[0-2]\d)\b/);
      if (yf) year = parseInt(yf[1]);
    }
  ...
  if (listings.length === 0) {
    ...
    for (const block of fallbackBlocks) {
      ...
      listings.push({
        ...
        brand,
        model,
        trim: trim || extractTrimFromTitle(title, brand, model),
        year: year || new Date().getFullYear(),
        price,
        km,
        fuel: detectFuel(block),
        transmission: detectTransmission(block),
        power: null,
        color: detectColor(block),
        doors: null,
        body_type: bodyType,
  ...
  if (listings.length === 0) {
    ...
    for (const match of urlMatches) {
      ...
      if (linkT && linkT[1].toLowerCase().includes(brandPrefix))
        title = linkT[1].replace(/\*\*/g, "").trim();
      ...
      listings.push({
        ...
        km,
        fuel: detectFuel(ctx),
        transmission: detectTransmission(ctx),
        power: null,
        color: detectColor(ctx),
        doors: null,
        body_type: detectBodyType(title),
        source: "autoscout24",
        source_url: sourceUrl,
        image_url: imageUrl,
        location: null,
  ...
  console.log(
    `AutoScout parser: ${listings.length} listings found (md length: ${markdown.length})`,
  );
  return listings;
}
...
// ==========================================
function parseAutomobileListings(
  markdown: string,
  brand: string,
  model: string,
  trim: string | null,
  ...
  for (const section of sections) {
    ...
    const srcUrlMatch = section.match(
      /\[!\[[^\]]*\]\([^)]*\)\]\((https?:\/\/[^\s)]*automobile\.it[^\s)]+)\)/,
    );
    ...
    if (!year) {
      const ym = section.match(/\b(20[0-2]\d)\b/);
    ...
    listings.push({
      ...
      image_url: imageUrl,
      location: null,
      is_new: km < 100 || /\bNuov[ao]\b/i.test(section),
  ...
  if (listings.length === 0) {
    ...
    for (const match of markdown.matchAll(urlRegex)) {
      ...
      if (yM) year = parseInt(yM[1]);
      if (!year) {
        const yf = ctx.match(/\b(20[0-2]\d)\b/);
        if (yf) year = parseInt(yf[1]);
      }
      let km = 0;
...
// ==========================================
function parseBrumBrumListings(
  markdown: string,
  brand: string,
  model: string,
  trim: string | null,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const linkMatch = line.match(
      /\[([^\]]+)\]\((https:\/\/www\.brumbrum\.it\/(?:auto|usato|catalogo)\/[^\s)]+)\)/,
    );
  ...
  for (const candidate of listings) {
    ...
    if (existingIdx === -1) {
      ...
      if (!allSources.includes(candidate.source)) {
        result[existingIdx] = {
          ...existing,
          extra_data: { ...existing.extra_data, all_sources: [...allSources, candidate.source] },
        };
      }
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const sources: string[] = filters?.sources?.length
      ? filters.sources
    ...
    if (!brand && !bodyTypeFilter && !fuelFilter && !priceMax && !kmMax && !yearMin) {
      return new Response(
        JSON.stringify({ success: false, error: "Specifica una marca o almeno un filtro" }),
    ...
    const scrapeJobs: {
      url: string;
      parser: (md: string) => ParsedListing[];
      waitFor?: number;
    ...
    if (!brand) {
      if (sources.includes("autoscout24")) {
        for (let page = 1; page <= 2; page++) {
          ...
          scrapeJobs.push({
            url,
            parser: (md) => parseAutoScoutListings(md, "", "", null),
            waitFor: 8000,
          });
        }
      }
      ...
      if (sources.includes("autoscout24")) {
        for (let page = 1; page <= 2; page++) {
          ...
          scrapeJobs.push({
            url,
            parser: (md) => parseAutoScoutListings(md, brand, model, trim),
            waitFor: 8000,
          });
        }
      }
      ...
      if (sources.includes("automobile")) {
        for (let page = 1; page <= 2; page++) {
          ...
          scrapeJobs.push({
            url,
            parser: (md) => parseAutomobileListings(md, brand, model, trim),
            premiumProxy: true,
          });
        }
      }
      ...
      if (sources.includes("brumbrum")) {
        for (let page = 1; page <= 2; page++) {
          ...
          scrapeJobs.push({
            url,
            parser: (md) => parseBrumBrumListings(md, brand, model, trim),
            premiumProxy: true,
          });
        }
      }
    ...
    const results = await Promise.allSettled(
      scrapeJobs.map(async (job, i) => {
        ...
        const md = await Promise.race([
          scrapeUrl(apiKey, job.url, job.waitFor, job.premiumProxy),
          jobTimeout,
        ]).catch((err) => {
          console.warn("Job failed/timeout:", err.message);
          return "";
        });
        if (!md) return [];
    ...
    if (unique.length > 0) {
      ...
      console.log(
        `Sample: "${sample.title}" price=${sample.price} km=${sample.km} year=${sample.year} url=${sample.source_url}`,
      );
    }
