...
function parseWithRegex(query: string): Filters {
  ...
  const priceMaxM = q.match(
    /(?:sotto|massimo|max|fino\s*a|entro|meno\s*di)\s*€?\s*([\d.,]+)\s*(?:k|mila)?(?:\s*€)?/,
  );
  if (priceMaxM) {
  ...
  const kmM = q.match(
    /(?:meno\s*di|max|massimo|sotto|entro)\s*([\d.,]+)\s*(?:k\s*)?(?:km|chilometri)/,
  );
  if (kmM) {
  ...
  const yearAfterM = q.match(
    /(?:dopo\s*il|dal?|immatricolata?\s*(?:dal?|dopo\s*il|nel)?|anno\s*(?:dal?|dopo))\s*(20\d{2})/,
  );
  ...
  if (yearRangeM) {
    filters.yearMin = parseInt(yearRangeM[1]);
    filters.yearMax = parseInt(yearRangeM[2]);
  }
  ...
  else if (/\bstation\s*wagon\b|\b\bsw\b|\bavant\b|\btouring\b|\bbreak\b/.test(q))
    filters.bodyType = "Station Wagon";
  else if (/\bberlina\b|\bsedan\b|\bhatchback\b/.test(q)) filters.bodyType = "Berlina";
  else if (/\bcabrio\b|\bcabriolet\b|\bdecapottabile\b|\bspider\b/.test(q))
    filters.bodyType = "Cabrio";
  ...
  const locM = q.match(
    /(?:zona\s+|vicino\s+a\s+|nei\s+pressi\s+di\s+|\ba\s+|\bin\s+)([a-zàèéìòù][a-zàèéìòù\s]{1,20}?)(?:\s*,|\s+con|\s+diesel|\s+benzina|\s+suv|\s*$)/,
  );
  if (locM) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!query?.trim()) {
      return new Response(JSON.stringify({ success: false, error: "Query vuota" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
