...
describe('useCompare', () => {
  ...
  it('adds a car', () => {
    const { result } = renderHook(() => useCompare(), { wrapper });
    act(() => { result.current.addToCompare('car-1'); });
  ...
  it('removes a car', () => {
    const { result } = renderHook(() => useCompare(), { wrapper });
    act(() => { result.current.addToCompare('car-1'); });
    act(() => { result.current.removeFromCompare('car-1'); });
  ...
  it('clears all cars', () => {
    const { result } = renderHook(() => useCompare(), { wrapper });
    act(() => { result.current.addToCompare('car-1'); });
    act(() => { result.current.addToCompare('car-2'); });
  ...
  it('enforces max 3 cars', () => {
    ...
    act(() => { result.current.addToCompare('car-1'); });
    act(() => { result.current.addToCompare('car-2'); });
    act(() => { result.current.addToCompare('car-3'); });
    act(() => { result.current.addToCompare('car-4'); });
